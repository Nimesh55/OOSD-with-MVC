...
class Executive_Controller extends Executive_Model
{
    ...
    public function setUpDetails()
    {
        ...
        $this->executive->setValues(
            $details['user_id'],
            $details['executive_no'],
            $details['first_name'],
            $details['last_name'],
            $details['address'],
            $details['telephone'],
    ...
    {
        if (
            empty($details['reason']) || empty($details['start_date']) || empty($details['end_date']) || empty($details['start_time']) ||
            ...
            $_SESSION["error"] = 'Required Field Empty';
        } elseif ((strtotime($details['start_date']) < strtotime(Timer::getInstance()->get_current_date())) || (strtotime($details['start_date']) > strtotime($details['end_date']))) {
        ...
        } elseif (Timer::getInstance()->my_date_diff($details['start_date'], $details['end_date'], 150)) {
            $_SESSION["error"] = 'Maximum of 150 days is Allowed';
        }
